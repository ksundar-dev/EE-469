...
module FlagsReg (negative_out, overflow_out, negative_in, overflow_in, FlagWrite, clk);
	...
	D_FF nff (.q(negative_out), .d(n_muxed), .reset(1'b0), .clk(clk));

	MUX2to1_1bit omux (.out(o_muxed), .a(overflow_out), .b(overflow_in), .sel(FlagWrite));
	D_FF off (.q(overflow_out), .d(o_muxed), .reset(1'b0), .clk(clk));
